...
#define DEBUG(...)                                                          \
    std::cerr << "\033[1;31mDEBUG: \033[0;31m{ (" << #__VA_ARGS__ << ") = " \
              << std::make_tuple(__VA_ARGS__) << " }\033[0m\n"
...
typename std::enable_if<i == sizeof...(tuple_types), void>::type tuple_log(
    std::ostream& os, std::tuple<tuple_types...> t) {
    return;
}

template <size_t i = 0, typename... tuple_types>
    typename std::enable_if <
    i<sizeof...(tuple_types), void>::type tuple_log(
        std::ostream& os, std::tuple<tuple_types...> t) {
    os << std::get<i>(t);
    if (i + 1 == sizeof...(tuple_types)) return;
    os << ", ";
    tuple_log<i + 1>(os, t);
}
